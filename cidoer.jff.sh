...
define_cidoer_jff() {
  ...
  do_print_24bit_bitmap() {
    ...
    local -r width_hex=$(xxd -p -l 4 -s 0x12 "$bmp_file")
    local -r height_hex=$(xxd -p -l 4 -s 0x16 "$bmp_file")
    local -r offset_hex=$(xxd -p -l 4 -s 0x0a "$bmp_file")
    local -r size_hex=$(xxd -p -l 4 -s 0x22 "$bmp_file")
    ...
    local row row_data col
    for ((row = height - 1; row >= 0; row--)); do
      local row_offset=$((data_offset + row * row_bytes))
      row_data=$(xxd -p -l $((width * 3)) -s "$row_offset" "$bmp_file" | tr -d '\n')
